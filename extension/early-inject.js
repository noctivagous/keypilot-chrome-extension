...
// KP_EARLY_INJECT_UI_END
  ...
  function renderEarlyKeyboard(container) {
    if (!container) return;
    const doc = container.ownerDocument || document;
    ensureKeybindingsUiStylesInjected(doc);

    // If already rendered, don't redo work.
    const existing = container.querySelector(':scope > .keyboard-visual');
    if (existing && existing.dataset && existing.dataset.kpKeyboardBuilt === 'true') return;
    ...
    for (const row of layout) {
      ...
      for (const item of row) {
        ...
        const binding = EARLY_KEYBINDINGS[item.id];
  ...
  function initEarlyStorage() {
    ...
    if (typeof chrome !== 'undefined' && chrome.storage) {
      ...
      chrome.storage.sync.get(['keypilot_enabled', KEYBOARD_HELP_STORAGE_KEY], (result) => {
