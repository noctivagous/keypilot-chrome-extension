...
- Reader Mode overlay on <kbd>P</kbd> (right-handed Browsing) / <kbd>I</kbd> (left-handed). Open Popover is no longer on a built-in key; bind it in Layout Editor. A toolbar under the title bar toggles article images; the choice is stored in `chrome.storage`.
