...
export function getActionIconDataUri(actionId, opts = {}) {
  ...
  const pathD = iconName === 'gear'
    ? ACCURATE_GEAR_PATH_16
    : (iconName ? FA_SOLID_PATHS[iconName] : null);
  if (!pathD) return '';
  return faIconDataUri(
    pathD,
    opts.fill || 'white',
    iconName === 'gear' ? '0 0 16 16' : undefined
  );
}
...
function getKeyboardKeyIconCss() {
  ...
  for (const [name, pathD] of Object.entries(FA_SOLID_PATHS)) {
    iconUris[name] = name === 'gear'
      ? faIconDataUri(ACCURATE_GEAR_PATH_16, 'black', '0 0 16 16')
      : faIconDataUri(pathD);
  }
