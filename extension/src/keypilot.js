...
import { elementFromPointDeep } from './utils/scroll-at-point.js';
...
import { withNavigationHandlers } from './modules/navigation-handlers.js';
import { withActivationHandlers } from './modules/activation-handlers.js';

export class KeyPilot extends withActivationHandlers(withNavigationHandlers(EventManager)) {
  constructor() {
    super();
  ...
  async completeSelection() {
    ...
    // Apply the live caret before the complete flag (updateSelection no-ops while
    // completing). Bypass the 2px mousemove throttle so finish uses the current cursor.
    if (selectionMode === 'character' || selectionMode === 'rectangle') {
      try {
        this.updateSelection({ force: true });
      } catch (e) {
        console.warn('[KeyPilot] Error applying latest caret before complete:', e);
      }
    }
