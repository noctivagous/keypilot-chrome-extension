...
function parseArgs(argv) {
  ...
  const channel = argv.find((arg) => CHANNELS.has(arg));
  return { channel, skipBuild };
}
...
function isVirtualWarPath(resource, config) {
...
export async function packageChannel(channel, { skipBuild = false } = {}) {
  if (!CHANNELS.has(channel)) {
    throw new Error(`Unknown package channel "${channel}". Use: ${[...CHANNELS].join(', ')}`);
  }
  ...
  function patchStagedManifest() {
    ...
    if (description.length > config.descriptionMaxLength) {
      throw new Error(
        `${label} description is ${description.length} characters; max is ${config.descriptionMaxLength}`
      );
    }
  ...
  function validateStagedManifest(manifest) {
    ...
    for (const resource of collectManifestPaths(manifest)) {
      if (isVirtualWarPath(resource, config)) continue;
    ...
    if (errors.length) {
      throw new Error(`${label} package validation failed:\n- ${errors.join('\n- ')}`);
    }
  }
...
if (isDirectRun) {
  const { channel, skipBuild } = parseArgs(process.argv.slice(2));
  if (!channel) {
    console.error('Usage: node scripts/package-channel.mjs <opera|chrome> [--skip-build]');
    process.exit(1);
  }
  packageChannel(channel, { skipBuild }).catch((err) => {
